...
class VisitorWiz():
    ...
    def move_old_data(self):
        ...
        cursor.execute(
            f"SELECT * FROM attendance WHERE date != '{self.date}'")
    ...
    def mark_attendance(self, encode_face):
        ...
        cursor.execute(
            f"SELECT * FROM attendance WHERE name = '{self.name}' AND date = '{self.date}'")
        result = cursor.fetchall()

        if len(result) == 0:
            cursor.execute(
                f"INSERT INTO attendance (name, face, date, status, attendanceTime) VALUES ('{self.name}', '{encode_face}', '{self.date}', 'Present', '{datetime.now().strftime('%I:%M %p')}')")
            self.database.commit()
    ...
    def init_ui(self):
        ...
        self.register_button.grid(row=1, column=0, rowspan=2)
